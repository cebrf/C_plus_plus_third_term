        ...
        for (int i = 0; i < Rows(); i++) {
            for (int j = 0; j < Cols(); j++) {
                if (this->operator()(i, j) != other(i, j)) {
                    std::cout << this->operator()(i, j) << " != " << other(i, j);
                    return false;
                }
            }
        }
...
int main(int argc, char** argv) {
    ...
    //auto a = RandomMat(4, 2, gen);
    //auto b = RandomMat(2, 2, gen);
    ...
    for (int tcount = 1; tcount <= 10; tcount++) { //int tcount = 2;
        int32_t mul_ms;
        DenseMat mat_res;
